...
BookExport 2.0.2 build 020 targets Paper 26.2 and Java 25 bytecode. Older Paper,
...
- A Java 25 or newer runtime for Paper and a Java 25 JDK/toolchain when building
  from source. Release verification builds with Oracle JDK 25.0.4.1 and also starts
  the server with Oracle JDK 26.0.2.1 without changing the Java 25 bytecode target.
...
For the Java 26 compatibility suite, keep that `JAVA_HOME` and `PATH`, then run:
...
The test classes still use Java 25 bytecode. Gradle discovers both explicitly
supplied JDKs without downloading a replacement. Use equivalent installed paths
on other machines. The live server uses Java 26.
...
```text
build/libs/1MB-BookExport-v2.0.2-020-j25-26.2.jar
```
...
2. Copy `1MB-BookExport-v2.0.2-020-j25-26.2.jar` into the server's `plugins/`
   directory.
...
2. Back up the existing BookExport JAR, `plugins/BookExport/config.yml`, staging,
   ...
   recovery set.
3. Remove the old BookExport JAR from `plugins/` and install the build 020 main JAR.
...
1. Confirm the startup log contains an enabled line for BookExport `2.0.2`, build
   `020`, artifact `1MB-BookExport-v2.0.2-020-j25-26.2.jar`, Java target `25`,
...
3. Run `/bookexport info` and `/bookexport version`; confirm both report build
   `020`, Paper `26.2` stable build `84`, exact compile API
